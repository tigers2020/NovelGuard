...
def debug_log(
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs):
            ...
            # ENTRY 로그
            params_summary = (
                _summarize_params(args, kwargs, max_param_length) if include_params else "..."
            )
            log_sink.write(
                LogEntry(
            ...
            try:
                ...
                # EXIT 로그
                result_summary = (
                    _summarize_result(result, max_param_length) if include_result else "..."
                )
                log_sink.write(
                    LogEntry(
                        timestamp=datetime.now(),
...
def debug_step(
    log_sink: Optional[ILogSink], step_name: str, details: Optional[dict[str, Any]] = None
) -> None:
    """단계별 디버깅 로그.
...
@contextmanager
def debug_context(
    log_sink: Optional[ILogSink], operation_name: str, details: Optional[dict[str, Any]] = None
):
    """디버깅 컨텍스트 매니저.
