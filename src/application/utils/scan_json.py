...
def serialize_scan_result_to_json(
    result: ScanResult, folder_path: Optional[Path] = None
) -> Dict[str, Any]:
    ...
    for entry in result.entries:
        file_dict: Dict[str, Any] = {
            ...
            "size": entry.size,
            "mtime": (
                entry.mtime.isoformat() if isinstance(entry.mtime, datetime) else str(entry.mtime)
            ),
...
def save_scan_result_to_json(
    result: ScanResult, output_path: Path, folder_path: Optional[Path] = None
) -> None:
