"""Build DuplicateGroupDetail for near duplicate groups (PR-19)."""
...
from domain.duplicate_near import NEAR_DUP_THRESHOLD, NearDuplicateGroup
...
def build_near_group_detail(
    group_id: str,
    *,
    near_group: NearDuplicateGroup | None,
    review_rows: list[dict[str, Any]],
    files_by_id: dict[str, FileRecord],
    quality_by_path: dict[str, list[dict[str, Any]]],
) -> dict[str, Any]:
    gid = group_id.strip()
    if near_group is None or near_group.group_id != gid:
        return {
            "status": "not_found",
            "groupId": gid,
            "members": [],
    ...
    return {
        ...
        "evidence": {
            ...
            "maxSimilarity": near_group.max_similarity,
