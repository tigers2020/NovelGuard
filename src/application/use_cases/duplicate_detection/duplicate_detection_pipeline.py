...
from application.use_cases.duplicate_detection.stages.group_creation_stage import GroupCreationStage
from application.use_cases.duplicate_detection.stages.near_duplicate_stage import NearDuplicateStage
...
class DuplicateDetectionPipeline:
    ...
    def __init__(
        ...
        stages: list[PipelineStage] = [
            ...
            FileMappingStage(file_data_store=file_data_store, log_sink=log_sink),
            BlockingStage(blocking_service=blocking_service, log_sink=log_sink),
            RelationDetectionStage(containment_detector=containment_detector, log_sink=log_sink),
        ]
        if exact_detector is not None:
            stages.append(ExactDuplicateStage(exact_detector=exact_detector, log_sink=log_sink))
        if near_detector is not None:
            stages.append(NearDuplicateStage(near_detector=near_detector, log_sink=log_sink))
        stages.append(GroupCreationStage(file_data_store=file_data_store, log_sink=log_sink))
        self._stages = stages
    ...
    def execute(
        ...
        for stage_idx, stage in enumerate(self._stages):
            ...
            if progress_callback:
                progress_callback(stage_idx, len(self._stages), f"{stage.name} 시작...")
