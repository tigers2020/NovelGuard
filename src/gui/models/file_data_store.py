...
class FileDataStore(QObject):
    ...
    def __init__(
        self, parent: Optional[QObject] = None, log_sink: Optional["ILogSink"] = None
    ) -> None:
        """파일 데이터 저장소 초기화.
    ...
    def clear(self) -> None:
        ...
        if self._log_sink:
            ...
            debug_step(self._log_sink, "file_data_store_clear", {"files_count": len(self._files)})
    ...
    def add_file(self, entry: FileEntry) -> FileData:
        ...
        file_data = FileData(entry=entry, file_id=file_id)
    ...
    def set_duplicate_groups_batch(
        self, updates: list[tuple[int, Optional[int], bool, Optional[float]]]
    ) -> None:
        """중복 그룹 정보 배치 설정.
        ...
        if changed_ids:
            ...
            logger.debug(
                "FileDataStore.set_duplicate_groups_batch: emitting files_updated_batch (%s ids)",
                len(changed_ids),
            )
            self.files_updated_batch.emit(changed_ids)
            logger.debug(
                "FileDataStore.set_duplicate_groups_batch: files_updated_batch emitted",
            )
            if self._log_sink:
                from application.utils.debug_logger import debug_step
    ...
    def add_integrity_issue(self, file_id: int, message: str, severity: str) -> None:
        ...
        current_severity_order = severity_order.get(file_data.integrity_severity or "", 0)
    ...
    def set_encoding(self, file_id: int, encoding: str, confidence: Optional[float] = None) -> None:
        ...
        """
        self.update_file(file_id, encoding=encoding, encoding_confidence=confidence)
