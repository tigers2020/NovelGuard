...
from gui.views.components.file_list_constants import (
    FileListColumns,
    FileListRoles,
    FileListUpdatePolicy,
)
...
class FileListTableWidget(QWidget):
    ...
    def _setup_ui(self) -> None:
        ...
        header.setSectionResizeMode(
            FileListColumns.FILE_NAME, QHeaderView.ResizeMode.ResizeToContents
        )
        header.setSectionResizeMode(FileListColumns.FILE_PATH, QHeaderView.ResizeMode.Stretch)
        header.setSectionResizeMode(
            FileListColumns.FILE_SIZE, QHeaderView.ResizeMode.ResizeToContents
        )
        header.setSectionResizeMode(
            FileListColumns.MODIFIED_AT, QHeaderView.ResizeMode.ResizeToContents
        )
        header.setSectionResizeMode(
            FileListColumns.EXTENSION, QHeaderView.ResizeMode.ResizeToContents
        )
        header.setSectionResizeMode(
            FileListColumns.ENCODING, QHeaderView.ResizeMode.ResizeToContents
        )
        header.setSectionResizeMode(
            FileListColumns.DUPLICATE_GROUP, QHeaderView.ResizeMode.ResizeToContents
        )
        header.setSectionResizeMode(
            FileListColumns.CANONICAL, QHeaderView.ResizeMode.ResizeToContents
        )
        header.setSectionResizeMode(
            FileListColumns.INTEGRITY, QHeaderView.ResizeMode.ResizeToContents
        )
        header.setSectionResizeMode(
            FileListColumns.ATTRIBUTES, QHeaderView.ResizeMode.ResizeToContents
        )
    ...
    def _connect_signals(self) -> None:
        ...
        connected = self._data_store.files_updated_batch.connect(self._on_files_updated_batch)
        logger.debug(
            "FileListTableWidget._connect_signals: files_updated_batch connected=%s", connected
        )
    ...
    def _set_file_row_data(self, row: int, file_data: FileData) -> None:
        ...
        # 경로
        self._table.setItem(
            row, FileListColumns.FILE_PATH, QTableWidgetItem(self._path_display_string(file_data))
        )
        ...
        # 인코딩
        self._table.setItem(
            row, FileListColumns.ENCODING, QTableWidgetItem(self._encoding_display_text(file_data))
        )
        ...
        self._table.setItem(
            row,
            FileListColumns.INTEGRITY,
            QTableWidgetItem(self._integrity_display_text(file_data)),
        )

        # 속성
        self._table.setItem(
            row,
            FileListColumns.ATTRIBUTES,
            QTableWidgetItem(self._attributes_display_text(file_data)),
        )
