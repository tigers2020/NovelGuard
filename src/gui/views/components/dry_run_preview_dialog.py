...
class DryRunPreviewDialog(QDialog):
    ...
    def __init__(
        self, operations: list[MoveOperation], scan_folder: Path, parent: Optional[QWidget] = None
    ) -> None:
    ...
    def _setup_ui(self) -> None:
        ...
        # 정보 라벨
        info_label = QLabel(f"총 {len(self._operations)}개 파일이 duplicate/ 폴더로 이동됩니다.")
        ...
        table.setHorizontalHeaderLabels(["원본 경로", "이동할 경로", "크기", "그룹 ID"])
        ...
        for row, operation in enumerate(self._operations):
            ...
            # 그룹 ID
            group_text = (
                str(operation.duplicate_group_id)
                if operation.duplicate_group_id is not None
                else "-"
            )
