...
class SettingsTab(BaseTab):
    ...
    def _create_duplicate_settings_group(self) -> QGroupBox:
        ...
        self._similarity_slider.setOrientation(Qt.Orientation.Horizontal)
        self._similarity_slider.setRange(
            Constants.SIMILARITY_MIN_PERCENT, Constants.SIMILARITY_MAX_PERCENT
        )
        ...
        self._conflict_policy.addItems(
            ["건너뛰기 (Skip)", "접미사 추가 (Rename)", "덮어쓰기 (Overwrite)", "병합 (Merge)"]
        )
        self._conflict_policy.setCurrentIndex(
            Constants.DEFAULT_CONFLICT_POLICY_INDEX
        )  # 접미사 추가
    ...
    def _on_save_settings(self) -> None:
        ...
        try:
            ...
            self._settings.setValue(
                SETTINGS_KEY_EXTENSION_FILTER, self._extension_input.text().strip()
            )
            self._settings.setValue(SETTINGS_KEY_INCLUDE_SUBDIRS, self._include_subdirs.isChecked())
            self._settings.setValue(
                SETTINGS_KEY_INCREMENTAL_SCAN, self._incremental_scan.isChecked()
            )
            self._settings.setValue(SETTINGS_KEY_INCLUDE_HIDDEN, self._include_hidden.isChecked())
            self._settings.setValue(
                SETTINGS_KEY_INCLUDE_SYMLINKS, self._include_symlinks.isChecked()
            )
            ...
            self._settings.setValue(
                SETTINGS_KEY_INCLUDE_RELATION, self._include_relation.isChecked()
            )
            self._settings.setValue(
                SETTINGS_KEY_SIMILARITY_PERCENT, self._similarity_slider.value()
            )
            self._settings.setValue(
                SETTINGS_KEY_CONFLICT_POLICY, self._conflict_policy.currentIndex()
            )

            # 성능 설정 저장
            self._settings.setValue(
                SETTINGS_KEY_WORKER_THREADS, self._worker_threads.currentIndex()
            )
            ...
            # 오류 메시지 표시
            QMessageBox.warning(
                self, "설정 저장 오류", f"설정 저장 중 오류가 발생했습니다:\n{str(e)}"
            )
    ...
    def _load_settings(self) -> None:
        ...
        self._include_subdirs.setChecked(
            cast(bool, self._settings.value(SETTINGS_KEY_INCLUDE_SUBDIRS, True, type=bool))
        )
        self._incremental_scan.setChecked(
            cast(bool, self._settings.value(SETTINGS_KEY_INCREMENTAL_SCAN, True, type=bool))
        )
        self._include_hidden.setChecked(
            cast(bool, self._settings.value(SETTINGS_KEY_INCLUDE_HIDDEN, False, type=bool))
        )
        self._include_symlinks.setChecked(
            cast(bool, self._settings.value(SETTINGS_KEY_INCLUDE_SYMLINKS, True, type=bool))
        )

        # 중복 탐지 설정 로드
        self._exact_duplicate.setChecked(
            cast(bool, self._settings.value(SETTINGS_KEY_EXACT_DUPLICATE, True, type=bool))
        )
        self._near_duplicate.setChecked(
            cast(bool, self._settings.value(SETTINGS_KEY_NEAR_DUPLICATE, True, type=bool))
        )
        self._include_relation.setChecked(
            cast(bool, self._settings.value(SETTINGS_KEY_INCLUDE_RELATION, True, type=bool))
        )

        similarity_percent = cast(
            int,
            self._settings.value(
                SETTINGS_KEY_SIMILARITY_PERCENT,
                Constants.DEFAULT_SIMILARITY_PERCENT,
                type=int,
            ),
        )
        if (
            Constants.SIMILARITY_MIN_PERCENT
            <= similarity_percent
            <= Constants.SIMILARITY_MAX_PERCENT
        ):
            self._similarity_slider.setValue(similarity_percent)
            self._similarity_label.setText(f"{similarity_percent}%")

        conflict_policy_index = cast(
            int,
            self._settings.value(
                SETTINGS_KEY_CONFLICT_POLICY,
                Constants.DEFAULT_CONFLICT_POLICY_INDEX,
        ...
        # 성능 설정 로드
        worker_threads_index = cast(
            int, self._settings.value(SETTINGS_KEY_WORKER_THREADS, 1, type=int)
        )
