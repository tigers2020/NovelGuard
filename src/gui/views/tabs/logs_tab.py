...
class LogsTab(BaseTab):
    ...
    def __init__(self, parent=None, log_sink: Optional[ILogSink] = None) -> None:
        """로그 탭 초기화.
        ...
        if hasattr(self, "_log_console") and self._log_console:
            ...
            if not logs:
                self._log_console.appendPlainText(
                    f"로그가 없습니다. (필터: {level_filter if hasattr(self, '_level_filter') and self._level_filter else '모두'})"
                )
            else:
    ...
    def _on_export_logs(self) -> None:
        """로그 내보내기 핸들러."""
        if not self._log_sink or not hasattr(self._log_sink, "get_logs"):
            QMessageBox.warning(self, "로그 내보내기 오류", "로그 싱크가 연결되지 않았습니다.")
            return
        ...
        file_path, _ = QFileDialog.getSaveFileName(
            self, "로그 내보내기", default_filename, "텍스트 파일 (*.txt);;모든 파일 (*.*)"
        )
        ...
        try:
            ...
            if not logs:
                QMessageBox.information(self, "로그 내보내기", "내보낼 로그가 없습니다.")
                return
            ...
            QMessageBox.critical(
                self, "로그 내보내기 오류", f"로그 내보내기 중 오류가 발생했습니다:\n{str(e)}"
            )
