...
class SQLiteIndexRepository:
    ...
    def start_run(self, request: ScanRequest) -> int:
        ...
        try:
            ...
            if run_id is None:
                raise RuntimeError("SQLite INSERT did not produce lastrowid")

            debug_step(self._log_sink, "start_run_complete", {"run_id": run_id})
    ...
    def finalize_run(self, run_id: int, summary: RunSummary) -> None:
        ...
        try:
            ...
            debug_step(self._log_sink, "finalize_run_complete", {"run_id": run_id})
        finally:
            conn.close()
    ...
    def get_latest_run_id(self) -> Optional[int]:
        ...
        try:
            ...
            debug_step(self._log_sink, "get_latest_run_id_complete", {"run_id": result})
    ...
    def get_run_summary(self, run_id: int) -> Optional[RunSummary]:
        ...
        Returns:
            Run 요약 정보. 없으면 None.
        """
        debug_step(self._log_sink, "get_run_summary_start", {"run_id": run_id})
    ...
    def get_ext_distribution(self, run_id: int) -> list[ExtStat]:
        ...
        Returns:
            확장자별 통계 리스트.
        """
        debug_step(self._log_sink, "get_ext_distribution_start", {"run_id": run_id})
    ...
    def list_files(
        ...
        if order_by not in self.ALLOWED_ORDER_BY:
            raise ValueError(
                f"Invalid order_by: {order_by}. Allowed: {list(self.ALLOWED_ORDER_BY.keys())}"
            )
