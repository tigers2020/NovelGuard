...
콘솔 미러링과 파일 쓰기 실패 알림은 의도적으로 ``print``를 쓴다.
(별도 logging 핸들러 없이 개발·디버그 시 stdout을 바로 보려는 어댑터 역할.)
"""
...
class InMemoryLogSink(QObject):
    ...
    def get_logs(self, job_id: Optional[int] = None, level: Optional[str] = None) -> list[LogEntry]:
        """로그 목록 조회.
            ...
            if entry.context:
                try:
                    ...
                    context_json = json.dumps(
                        entry.context, ensure_ascii=False, separators=(",", ":")
                    )
                    context_str = f" | {context_json}"
                except (TypeError, ValueError):
                    # JSON 변환 실패 시 문자열로 표시
