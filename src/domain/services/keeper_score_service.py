...
class KeeperScoreService:
    ...
    def calculate_keeper_score(
        ...
        if parse_result.is_complete:
            score += DetectionDefaults.SCORE_COMPLETE_TAG
        ...
        if parse_result.confidence < DetectionDefaults.CONFIDENCE_THRESHOLD:
            score += DetectionDefaults.PENALTY_LOW_CONFIDENCE

        return score

    def select_keeper(
        self, files_with_parse: list[tuple[FileEntry, FilenameParseResult]]
    ) -> Optional[FileEntry]:
        """Keeper 선택.
        ...
        for file_entry, parse_result in files_with_parse:
            ...
            if file_entry.mtime == max_mtime:
                base_score += DetectionDefaults.SCORE_MTIME

            # +10: size 큰 쪽 (가장 큰 파일에만)
            if file_entry.size == max_size:
                base_score += DetectionDefaults.SCORE_SIZE
