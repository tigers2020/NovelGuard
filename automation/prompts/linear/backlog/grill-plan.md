...
**Skip:** Plan + grill APPROVED already in comments → `save_issue(Todo, labels+=auto:plan-done)` **one call**. STOP. → `linear/todo/write-task-list.md`.
...
2. APPROVED → `## Implementation Plan` (**Summary caveman** at top) → `save_issue(Todo, labels+=auto:plan-done)` **one call**. STOP. → `linear/todo/write-task-list.md`.
