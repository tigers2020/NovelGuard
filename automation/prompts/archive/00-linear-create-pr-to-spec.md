...
1. **Load** — `get_issue` + `list_comments`. Parse Problem, AC, `files_allowed`. Stay **Backlog**; add `auto:triaging`.
...
5. **Closeout — MUST** — `save_issue(state=Todo, labels+=auto:research-done)` in **one call**. STOP. → `linear/todo/write-spec.md`.
