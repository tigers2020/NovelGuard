...
class TestFileRepositoryProtocol:
    ...
    def test_file_repository_has_all_methods(self):
        ...
        for method_name in required_methods:
            assert hasattr(repo, method_name), f"FileRepository에 {method_name} 메서드가 없습니다"
            assert callable(
...
class TestHashServiceProtocol:
    ...
    def test_hash_calculator_has_required_methods(self):
        ...
        required_methods = ["calculate_md5", "calculate_sha256", "calculate_hash"]

        for method_name in required_methods:
            assert hasattr(
                HashCalculator, method_name
            ), f"HashCalculator에 {method_name} 메서드가 없습니다"
            assert callable(
                getattr(HashCalculator, method_name)
    ...
    def test_fingerprint_generator_has_required_methods(self):
        ...
        for method_name in required_methods:
            assert hasattr(
                FingerprintGenerator, method_name
            ), f"FingerprintGenerator에 {method_name} 메서드가 없습니다"
            assert callable(
                getattr(FingerprintGenerator, method_name)
    ...
    def test_hash_service_adapter_implements_protocol(self):
        ...
        for method_name in required_methods:
            assert hasattr(
                HashServiceAdapter, method_name
            ), f"HashServiceAdapter에 {method_name} 메서드가 없습니다"
            assert callable(
                getattr(HashServiceAdapter, method_name)
...
class TestEncodingDetectorProtocol:
    ...
    def test_encoding_detector_implements_protocol(self):
        ...
        required_methods = ["detect", "detect_from_bytes", "decode_text"]

        for method_name in required_methods:
            assert hasattr(
                EncodingDetector, method_name
            ), f"EncodingDetector에 {method_name} 메서드가 없습니다"
            assert callable(
                getattr(EncodingDetector, method_name)
    ...
    def test_encoding_detector_uses_constants(self):
        ...
        assert hasattr(Constants, "MAX_SAMPLE_SIZE"), "Constants에 MAX_SAMPLE_SIZE 상수가 없습니다"
        assert isinstance(
            Constants.MAX_SAMPLE_SIZE, int
