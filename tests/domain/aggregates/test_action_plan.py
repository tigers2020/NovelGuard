...
class TestActionItemValidation:
    ...
    def test_invalid_action(self):
        ...
        with pytest.raises(ValueError, match="action must be one of"):
            ActionItem(action_id=1, file_id=10, action="INVALID")

    def test_invalid_risk(self):
        """잘못된 risk이면 실패."""
        with pytest.raises(ValueError, match="risk must be one of"):
            ActionItem(action_id=1, file_id=10, action="DELETE", risk="CRITICAL")
...
class TestActionItemProperties:
    ...
    def test_has_dependencies_true(self):
        """의존성 있음."""
        item = ActionItem(action_id=2, file_id=11, action="DELETE", depends_on=(1,))
...
class TestActionPlanValidation:
    ...
    def test_invalid_created_from(self):
        ...
        with pytest.raises(ValueError, match="created_from must be one of"):
            ActionPlan(plan_id=1, created_from="INVALID")
    ...
    def test_duplicate_action_ids(self):
        ...
        with pytest.raises(ValueError, match="Duplicate action_ids"):
            ActionPlan(plan_id=1, created_from="DUPLICATE", items=items)
...
class TestActionPlanMethods:
    ...
    def test_get_bytes_savable(self):
        """절감 가능 용량."""
        plan = ActionPlan(plan_id=1, created_from="DUPLICATE", summary={"bytes_savable": 1024000})
    ...
    def test_with_added_item(self):
        ...
        plan = ActionPlan(plan_id=1, created_from="DUPLICATE", items=(item1,))
...
class TestActionResultCreation:
    ...
    def test_create_success(self):
        """성공 결과 생성."""
        result = ActionResult(action_id=1, ok=True, before_path="/old/path.txt")
    ...
    def test_create_failure(self):
        """실패 결과 생성."""
        result = ActionResult(action_id=1, ok=False, error="Permission denied")
