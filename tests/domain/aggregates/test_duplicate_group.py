...
class TestDuplicateGroupCreation:
    ...
    def test_create_minimal(self):
        """최소 정보로 생성."""
        group = DuplicateGroup(group_id=1, group_type="EXACT")
    ...
    def test_create_with_reasons(self):
        """증거와 함께 생성."""
        group = DuplicateGroup(group_id=1, group_type="EXACT", reason_ids=(1, 2, 3))
...
class TestDuplicateGroupValidation:
    ...
    def test_invalid_confidence_too_low(self):
        ...
        with pytest.raises(ValueError, match="confidence must be between"):
            DuplicateGroup(group_id=1, group_type="EXACT", confidence=-0.1)

    def test_invalid_confidence_too_high(self):
        """confidence가 1.0 초과이면 실패."""
        with pytest.raises(ValueError, match="confidence must be between"):
            DuplicateGroup(group_id=1, group_type="EXACT", confidence=1.1)

    def test_invalid_group_type(self):
        """잘못된 group_type이면 실패."""
        with pytest.raises(ValueError, match="group_type must be one of"):
            DuplicateGroup(group_id=1, group_type="INVALID")

    def test_invalid_status(self):
        """잘못된 status이면 실패."""
        with pytest.raises(ValueError, match="status must be one of"):
            DuplicateGroup(group_id=1, group_type="EXACT", status="INVALID")
...
class TestDuplicateGroupImmutability:
    ...
    def test_frozen(self):
        ...
        group = DuplicateGroup(group_id=1, group_type="EXACT", member_ids=(10, 11))
...
class TestDuplicateGroupFunctionalUpdate:
    ...
    def test_with_canonical(self):
        """보존본 설정."""
        group = DuplicateGroup(group_id=1, group_type="EXACT", member_ids=(10, 11, 12))
    ...
    def test_with_added_reason(self):
        """증거 추가."""
        group = DuplicateGroup(group_id=1, group_type="EXACT", reason_ids=(1, 2))
...
class TestDuplicateGroupProperties:
    ...
    def test_has_canonical_true(self):
        """보존본 설정 여부 확인 (있음)."""
        group = DuplicateGroup(group_id=1, group_type="EXACT", member_ids=(10, 11), canonical_id=10)
    ...
    def test_is_verified(self):
        """검증 완료 상태 확인."""
        group = DuplicateGroup(group_id=1, group_type="EXACT", status="VERIFIED")

        assert group.is_verified is True

    def test_is_applied(self):
        """적용 완료 상태 확인."""
        group = DuplicateGroup(group_id=1, group_type="EXACT", status="APPLIED")
    ...
    def test_get_delete_candidate_ids(self):
        """삭제 후보 ID 목록."""
        group = DuplicateGroup(
            group_id=1, group_type="EXACT", member_ids=(10, 11, 12), canonical_id=10
        )
