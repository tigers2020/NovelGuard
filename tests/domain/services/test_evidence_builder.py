...
def create_test_file(
    ...
    """테스트용 File 생성."""
    path = FilePath(path=Path(f"/test/{name}"), name=name, ext=".txt", size=1000, mtime=1000.0)
    ...
    return File(file_id=create_file_id(file_id), path=path, metadata=metadata, hash_info=hash_info)
...
class TestEvidenceBuilderService:
    ...
    def test_build_hash_strong_evidence(self):
        ...
        detail = ComparisonDetail(
            result=ComparisonResult.IDENTICAL, similarity_score=1.0, matched_by="strong_hash"
        )

        evidence = self.service.build_from_comparison(file1, file2, detail, self.evidence_id_gen)
    ...
    def test_build_fingerprint_fast_evidence(self):
        ...
        detail = ComparisonDetail(
            result=ComparisonResult.IDENTICAL, similarity_score=1.0, matched_by="fingerprint"
        )
    ...
    def test_build_fingerprint_norm_evidence(self):
        ...
        detail = ComparisonDetail(
            result=ComparisonResult.SIMILAR, similarity_score=1.0, matched_by="fingerprint_norm"
        )
    ...
    def test_build_simhash_evidence(self):
        ...
        detail = ComparisonDetail(
            result=ComparisonResult.SIMILAR, similarity_score=0.93, matched_by="simhash"
        )
    ...
    def test_evidence_id_generation(self):
        ...
        evidence1 = self.service.build_from_comparison(file1, file2, detail1, self.evidence_id_gen)
        evidence2 = self.service.build_from_comparison(file3, file4, detail2, self.evidence_id_gen)
    ...
    def test_build_encoding_evidence(self):
        ...
        evidence = self.service.build_encoding_evidence(
            file, self.evidence_id_gen, issue_description="인코딩 감지 실패"
        )
    ...
    def test_build_multiple_evidences(self):
        ...
        evidences = self.service.build_multiple_evidences(comparisons, self.evidence_id_gen)
...
class TestEvidenceBuilderKindDetermination:
    ...
    def test_determine_kind_unknown(self):
        """알 수 없는 방법은 기본값."""
        detail = ComparisonDetail(
            result=ComparisonResult.DIFFERENT, similarity_score=0.0, matched_by=None
        )
