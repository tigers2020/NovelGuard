...
def create_test_file(
    ...
    if encoding is None and is_text:
        metadata = FileMetadata(
            is_text=True, encoding_detected=None, encoding_confidence=None, newline=None
        )
    elif is_text and encoding:
        metadata = FileMetadata.text_file(encoding=encoding, confidence=confidence)
    else:
...
class TestIntegrityCheckService:
    ...
    def test_check_normal_file(self):
        ...
        file = create_test_file(
            file_id=1, name="normal.txt", size=1000, encoding="utf-8", confidence=0.95
        )
    ...
    def test_check_empty_file(self):
        """빈 파일은 INFO 이슈."""
        file = create_test_file(
            file_id=1, name="empty.txt", size=0, encoding="utf-8", confidence=0.95  # 빈 파일
        )
    ...
    def test_check_no_encoding(self):
        """인코딩 정보 없으면 ERROR."""
        file = create_test_file(
            file_id=1, name="unknown.txt", size=1000, encoding=None, confidence=None  # 인코딩 없음
        )
    ...
    def test_check_low_confidence_error(self):
        """낮은 신뢰도 (< 0.5) → ERROR."""
        file = create_test_file(
            file_id=1, name="low_conf.txt", size=1000, encoding="utf-8", confidence=0.3  # 매우 낮음
        )
    ...
    def test_check_low_confidence_warn(self):
        """낮은 신뢰도 (0.5 ~ 0.7) → WARN."""
        file = create_test_file(
            file_id=1, name="medium_conf.txt", size=1000, encoding="utf-8", confidence=0.6  # 중간
        )
    ...
    def test_check_binary_file_no_encoding_check(self):
        """바이너리 파일은 인코딩 체크 안 함."""
        file = create_test_file(file_id=1, name="image.bin", size=1000, is_text=False)  # 바이너리
