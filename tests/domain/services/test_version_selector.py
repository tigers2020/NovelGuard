...
def create_test_file(
    ...
    """테스트용 File 생성."""
    path = FilePath(path=Path(f"/test/{name}"), name=name, ext=".txt", size=size, mtime=mtime)
    metadata = FileMetadata.text_file(encoding="utf-8", confidence=encoding_confidence)
    return File(
        file_id=create_file_id(file_id), path=path, metadata=metadata, hash_info=FileHashInfo()
    )
...
class TestVersionSelectionService:
    ...
    def test_select_auto_without_filename_pattern(self):
        """auto: 패턴 없으면 품질 점수."""
        files = [
            create_test_file(1, "fileA.txt", size=1000, mtime=1000.0, encoding_confidence=0.5),
            create_test_file(
                2, "fileB.txt", size=5000, mtime=3000.0, encoding_confidence=0.95  # 최고 품질
            ),
            create_test_file(3, "fileC.txt", size=3000, mtime=2000.0, encoding_confidence=0.8),
        ]
    ...
    def test_select_quality_strategy(self):
        """quality 전략."""
        files = [
            create_test_file(1, "file1.txt", size=1000, mtime=1000.0, encoding_confidence=0.5),
            create_test_file(
                2, "file2.txt", size=8000, mtime=5000.0, encoding_confidence=0.98  # 최고
            ),
            create_test_file(3, "file3.txt", size=5000, mtime=3000.0, encoding_confidence=0.8),
        ]
    ...
    def test_select_canonical_for_group(self):
        ...
        canonical_id = self.service.select_canonical_for_group(
            file_ids=[10, 11, 12], file_lookup=file_lookup, strategy="auto"
        )
    ...
    def test_select_canonical_for_group_empty(self):
        """빈 그룹이면 None."""
        canonical_id = self.service.select_canonical_for_group(
            file_ids=[], file_lookup={}, strategy="auto"
        )
...
class TestVersionSelectionServiceIntegration:
    ...
    def test_realistic_scenario(self):
        ...
        files = [
            # 오래된 버전 (v1)
            create_test_file(
                1, "소설_완결_v1.txt", size=50000, mtime=1000.0, encoding_confidence=0.7
            ),
            # 중간 버전 (v2)
            create_test_file(
                2, "소설_완결_v2.txt", size=52000, mtime=2000.0, encoding_confidence=0.85
            ),
            # 최신 버전 (v3) - 선택될 것
            create_test_file(
                3, "소설_완결_v3.txt", size=55000, mtime=3000.0, encoding_confidence=0.95
            ),
            # 버전 표시 없는 파일 (무시됨)
            create_test_file(
                4, "소설_완결.txt", size=60000, mtime=4000.0, encoding_confidence=0.99
            ),
        ]
