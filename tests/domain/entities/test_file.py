...
def test_file_creation():
    ...
    file_entity = File(file_id=file_id, path=path, metadata=metadata, hash_info=hash_info)
...
def test_update_hash_info():
    ...
    new_hash = FileHashInfo(hash_strong="xyz789", fingerprint_fast="fast123")
...
def _create_sample_file(size: int = 1024, is_text: bool = True) -> File:
    ...
    file_id = FileId(1)
    path = FilePath(
        path=Path("C:/test/file.txt"), name="file.txt", ext=".txt", size=size, mtime=1609459200.0
    )
