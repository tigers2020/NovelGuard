...
def test_file_path_validation_negative_size():
    ...
    with pytest.raises(ValueError, match="File size must be non-negative"):
        FilePath(
            path=Path("C:/test/file.txt"), name="file.txt", ext=".txt", size=-1, mtime=1609459200.0
        )
...
def test_file_path_validation_negative_mtime():
    ...
    with pytest.raises(ValueError, match="Modification time must be non-negative"):
        FilePath(path=Path("C:/test/file.txt"), name="file.txt", ext=".txt", size=1024, mtime=-1.0)
...
def test_mtime_datetime():
    ...
    file_path_zero = FilePath(
        path=Path("C:/test/file.txt"), name="file.txt", ext=".txt", size=1024, mtime=0.0
    )
...
def test_from_path_str():
    ...
    file_path = FilePath.from_path_str(path_str="C:/test/file.TXT", size=1024, mtime=1609459200.0)
...
def test_is_empty():
    ...
    # 빈 파일
    empty_file = FilePath(
        path=Path("C:/test/empty.txt"), name="empty.txt", ext=".txt", size=0, mtime=1609459200.0
    )
