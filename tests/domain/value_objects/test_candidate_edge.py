...
class TestCandidateEdgeCreation:
    ...
    def test_create_minimal(self):
        """최소 정보로 생성."""
        edge = CandidateEdge(a_id=10, b_id=11, relation="EXACT")
    ...
    def test_create_with_score(self):
        """점수와 함께 생성."""
        edge = CandidateEdge(a_id=10, b_id=11, relation="NEAR", score=0.95, evidence_id=1)
...
class TestCandidateEdgeValidation:
    ...
    def test_invalid_score_too_low(self):
        ...
        with pytest.raises(ValueError, match="score must be between"):
            CandidateEdge(a_id=10, b_id=11, relation="EXACT", score=-0.1)

    def test_invalid_score_too_high(self):
        """score가 1.0 초과이면 실패."""
        with pytest.raises(ValueError, match="score must be between"):
            CandidateEdge(a_id=10, b_id=11, relation="EXACT", score=1.1)

    def test_invalid_relation(self):
        """잘못된 relation이면 실패."""
        with pytest.raises(ValueError, match="relation must be one of"):
            CandidateEdge(a_id=10, b_id=11, relation="INVALID")

    def test_same_file_ids(self):
        """a_id와 b_id가 같으면 실패."""
        with pytest.raises(ValueError, match="a_id and b_id must be different"):
            CandidateEdge(a_id=10, b_id=10, relation="EXACT")
...
class TestCandidateEdgeProperties:
    ...
    def test_is_near(self):
        """유사 관계 확인."""
        edge = CandidateEdge(a_id=10, b_id=11, relation="NEAR")
    ...
    def test_is_containment_a_in_b(self):
        """포함 관계 확인 (A in B)."""
        edge = CandidateEdge(a_id=10, b_id=11, relation="CONTAINS_A_IN_B")
    ...
    def test_is_containment_b_in_a(self):
        """포함 관계 확인 (B in A)."""
        edge = CandidateEdge(a_id=10, b_id=11, relation="CONTAINS_B_IN_A")
