...
class TestEvidenceCreation:
    ...
    def test_create_minimal(self):
        """최소 정보로 생성."""
        evidence = Evidence(evidence_id=1, kind="HASH_STRONG")
    ...
    def test_create_with_detail(self):
        """상세 정보와 함께 생성."""
        detail = {"algorithm": "md5", "value": "abc123def456"}
        evidence = Evidence(evidence_id=1, kind="HASH_STRONG", detail=detail)

        assert evidence.detail == detail

    def test_create_with_similarity(self):
        """유사도 정보와 함께 생성."""
        evidence = Evidence(evidence_id=1, kind="SIMHASH", detail={"similarity": 0.93})
...
class TestEvidenceValidation:
    ...
    def test_invalid_kind(self):
        ...
        with pytest.raises(ValueError, match="kind must be one of"):
            Evidence(evidence_id=1, kind="INVALID")
    ...
    def test_detail_with_complex_object(self):
        ...
        with pytest.raises(ValueError, match="contains non-primitive object"):
            Evidence(evidence_id=1, kind="HASH_STRONG", detail={"file": MockFile()})
...
class TestEvidenceMethods:
    ...
    def test_get_match_spans_present(self):
        ...
        evidence = Evidence(evidence_id=1, kind="CONTAINMENT_RK", detail={"match_spans": spans})
    ...
    def test_has_detail_true(self):
        """상세 정보 존재 확인 (있음)."""
        evidence = Evidence(evidence_id=1, kind="HASH_STRONG", detail={"algorithm": "md5"})
...
class TestEvidenceKinds:
    ...
    def test_hash_strong(self):
        """HASH_STRONG 증거."""
        evidence = Evidence(
            evidence_id=1, kind="HASH_STRONG", detail={"algorithm": "md5", "value": "abc123"}
        )
    ...
    def test_fp_fast(self):
        """FP_FAST 증거."""
        evidence = Evidence(
            evidence_id=1, kind="FP_FAST", detail={"head": "abc", "mid": "def", "tail": "ghi"}
        )
    ...
    def test_norm_hash(self):
        """NORM_HASH 증거."""
        evidence = Evidence(evidence_id=1, kind="NORM_HASH", detail={"normalized_text": "..."})
    ...
    def test_simhash(self):
        """SIMHASH 증거."""
        evidence = Evidence(
            evidence_id=1, kind="SIMHASH", detail={"similarity": 0.93, "hamming_distance": 3}
        )
    ...
    def test_containment_rk(self):
        ...
        evidence = Evidence(
            evidence_id=1,
            kind="CONTAINMENT_RK",
            detail={"match_spans": [(1200, 5600)], "coverage": 0.85},
        )
    ...
    def test_text_diff(self):
        ...
        evidence = Evidence(
            evidence_id=1,
            kind="TEXT_DIFF",
            detail={"added_lines": 5, "removed_lines": 3, "diff_ratio": 0.92},
        )
