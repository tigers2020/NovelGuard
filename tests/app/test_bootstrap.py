...
def test_create_application_injects_dependencies(
    mock_usecase, mock_repo, mock_encoding, mock_app
) -> None:
...
def test_main_window_constructor_accepts_dependencies() -> None:
    ...
    # Mock 의존성 생성
    Mock(spec=ScanFilesUseCase)
    Mock(spec=FileRepository)
    logging.getLogger("test")

    # MainWindow 생성 (PySide6 QApplication이 필요하므로 Mock)
    # QMainWindow를 완전히 Mock하여 초기화 문제 방지
    with (
        patch("gui.views.main_window.QMainWindow") as mock_qmainwindow,
...
def test_main_window_no_direct_imports() -> None:
    ...
    assert has_type_checking or not any(
        "infra" in source or "domain" in source for _ in [1]
