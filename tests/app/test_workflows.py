    ...
    for node in ast.walk(execute_method):
        if isinstance(node, forbidden_nodes):
            ...
            raise AssertionError(
                f"ScanFlow에 금지된 {node_type} 노드 발견: " f"{node_src or 'N/A'}"
            )
...
def test_analysis_flow_contains_no_branching_logic() -> None:
    ...
    for node in ast.walk(execute_method):
        if isinstance(node, forbidden_nodes):
            ...
            node_src = ast.get_source_segment(class_src, node)
            raise AssertionError(
                f"AnalysisFlow에 금지된 {node_type} 노드 발견: " f"{node_src or 'N/A'}"
            )
        ...
        if isinstance(stmt, (ast.Return, ast.Assign, ast.Expr, ast.Pass)):
            ...
            if isinstance(stmt, ast.Expr) and isinstance(stmt.value, ast.Call):
                ...
                if not (
                    ...
                    and stmt.value.func.value.attr.endswith("_usecase")
                ):
                    node_src = ast.get_source_segment(class_src, stmt)
                    raise AssertionError(f"ScanFlow에 허용되지 않은 호출: " f"{node_src or 'N/A'}")
        elif isinstance(stmt, ast.FunctionDef):
            # 내부 함수 정의는 허용하지 않음
            node_src = ast.get_source_segment(class_src, stmt)
            raise AssertionError(
                f"ScanFlow에 허용되지 않은 내부 함수 정의: " f"{node_src or 'N/A'}"
            )
        ...
        if isinstance(stmt, (ast.Return, ast.Assign, ast.Expr, ast.Pass)):
            ...
            if isinstance(stmt, ast.Expr) and isinstance(stmt.value, ast.Call):
                ...
                if not (
                    ...
                    node_src = ast.get_source_segment(class_src, stmt)
                    raise AssertionError(
                        f"AnalysisFlow에 허용되지 않은 호출: " f"{node_src or 'N/A'}"
                    )
        elif isinstance(stmt, ast.FunctionDef):
            # 내부 함수 정의는 허용하지 않음
            node_src = ast.get_source_segment(class_src, stmt)
            raise AssertionError(
                f"AnalysisFlow에 허용되지 않은 내부 함수 정의: " f"{node_src or 'N/A'}"
            )
