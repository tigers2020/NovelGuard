...
def test_file_mapping_stage_execute():
    ...
    stage = FileMappingStage(file_data_store=file_data_store, log_sink=log_sink)
...
def test_file_mapping_stage_execute_high_failure_rate():
    ...
    file_entries = [
        FileEntry(
            path=Path(f"test{i}.txt"), size=100, mtime=datetime.now(), extension=".txt", file_id=i
        )
        for i in range(10)
    ]
    ...
    file_data_store.get_file_id_by_path.side_effect = [
        10 if i < 3 else None for i in range(10)  # 처음 3개만 매핑 성공
    ]
...
def test_file_mapping_stage_execute_no_file_data_store():
    ...
    stage = FileMappingStage(file_data_store=None, log_sink=log_sink)  # type: ignore
