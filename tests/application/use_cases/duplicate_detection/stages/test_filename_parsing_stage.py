...
def test_filename_parsing_stage_name():
    ...
    stage = FilenameParsingStage(filename_parser=filename_parser, index_repository=index_repository)
...
def test_filename_parsing_stage_execute():
    ...
    stage = FilenameParsingStage(
        filename_parser=filename_parser, index_repository=index_repository, log_sink=log_sink
    )
...
def test_filename_parsing_stage_execute_pagination():
    ...
    file_entry3 = FileEntry(
        path=Path("test3.txt"), size=300, mtime=datetime.now(), extension=".txt", file_id=3
    )
...
def test_filename_parsing_stage_execute_no_file_id():
    ...
    file_entry = FileEntry(
        path=Path("test.txt"), size=100, mtime=datetime.now(), extension=".txt", file_id=None
    )

    # Mock 설정
    index_repository.list_files.side_effect = [[file_entry], []]
...
def test_filename_parsing_stage_execute_parsing_error():
    ...
    # Mock 설정
    index_repository.list_files.side_effect = [[file_entry1, file_entry2], []]
    ...
    def mock_parse(path: Path) -> FilenameParseResult:
        ...
        return FilenameParseResult(
            original_path=path, original_name=path.stem, series_title_norm="test", confidence=0.9
        )
