...
def test_upsert_files(
    repository: SQLiteIndexRepository, scan_request: ScanRequest, file_entries: list[FileEntry]
) -> None:
    """upsert_files 테스트."""
    run_id = repository.start_run(scan_request)
...
def test_upsert_files_duplicate_path(
    repository: SQLiteIndexRepository, scan_request: ScanRequest, file_entries: list[FileEntry]
) -> None:
